...
def loader(path, version=None):
    ...
    if version:
        ...
        model_version = "model_{}".format(version)
        if model_version in model_dirs:
            model_path = os.path.join(path, "model_{}".format(version))
            model = joblib.load(os.path.join(model_path, "classifier.pkl"))
            vector_me = joblib.load(os.path.join(model_path, "feature.pkl"))
            return model, vector_me
        else:
        ...
        vector_me = joblib.load(os.path.join(latest_model, "feature.pkl"))
        return model, vector_me
